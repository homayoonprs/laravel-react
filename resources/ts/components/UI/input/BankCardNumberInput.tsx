...
import styles from './BankCardNumberInput.module.scss';
import {IBankCardNumber, IInput} from "./IInput";
...
const BankCardNumberInput = (props: IBankCardNumber) => {

    const inputsContainerRef = useRef<HTMLDivElement>(null)
    const bankLogoContainer = useRef<HTMLImageElement>(null)

    const inputs = Array.apply('', Array(4));

    let allInputsValue: string = '';

    const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        let {value} = event.target
        // @ts-ignore
        event.target.value = event.target.value.match(/^([^e+-]{0,4})/)[0]

        if (value.length == 4 && event.target.nextSibling){
            // @ts-ignore
        ...
        updateAllInputState()
        props.onChange && props.onChange({name: 'bank_card_number', value: allInputsValue})
    }
    ...
    const onKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
        ...
        if ( !value.length && 'previousSibling' in event.target){
            // @ts-ignore
    ...
    const updateAllInputState = () => {
        allInputsValue = ''
        if(inputsContainerRef.current){
            let {length} = inputsContainerRef.current.children
            for (let i = 0; i < length; i++){
                // @ts-ignore
                allInputsValue += inputsContainerRef.current.children.item(i).value
            }
        }
        let target = banks.filter(b => allInputsValue.startsWith(b.card_no.toString()))[0]
        if (target && bankLogoContainer.current){
            bankLogoContainer.current.src = target.bank_logo
        }else if (bankLogoContainer.current){
            bankLogoContainer.current.src = ''
        }
    ...
    return (
        <div className={styles.container}>
            <span className={cs(styles.label,'inline-block')}>{props.label}</span>
            <img className={cs(styles.label, 'inline-block w-5')} ref={bankLogoContainer}/>

            <div className={styles.inputContainer} ref={inputsContainerRef} dir="ltr">
                {inputs.map((i, key) => (
                    <input
                        {...props}
                        key={key}
                        id={key.toString()}
                        tabIndex={props.tabIndex ? props.tabIndex+key : undefined}
                        onKeyDown={onKeyDown}
                        onChange={handleInputChange}
                        className={styles[`${props.inputSize}`]}
                        type="text"
                        inputMode="numeric"
                    />
                ))}
